...
internal partial class MessagePool
{
	...
	private bool TranslateIDX(FileStream srcFS)
	{
		...
		string nameString = new string([.. Encoding.UTF8.GetString(streamBuffer).Where(c => !Path.GetInvalidFileNameChars().Contains(c))]).ToLower();
